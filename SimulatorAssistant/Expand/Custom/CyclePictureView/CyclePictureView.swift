...
// MARK: - 内部方法
    ...
    */
    func changePicture() {
        ...
        let currentIndex = Int(CGFloat(offsetIndex).truncatingRemainder(dividingBy: CGFloat(self.imageBox!.imageArray.count))  + 0.5)
...
extension CyclePictureView {
    ...
    func scrollViewDidScroll(scrollView: UIScrollView) {
        ...
        let offsetIndex = self.collectionView.contentOffset.x / self.flowLayout!.itemSize.width
        let currentIndex = Int(CGFloat(offsetIndex).truncatingRemainder(dividingBy: CGFloat(self.imageBox!.imageArray.count))  + 0.5) //Int(CGFloat(offsetIndex) % CGFloat(self.imageBox!.imageArray.count) + 0.5)
        pageControl.currentPage = currentIndex == self.imageBox!.imageArray.count ? 0 :currentIndex
    }
...
extension CyclePictureView {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if (self.imageBox?.imageArray.isEmpty)! {
            return cell
        }
...
//        self.delegate?.cyclePictureView(cyclePictureView: self, didSelectItemAtIndexPath: NSIndexPath(forItem: indexPath.item % self.imageBox!.imageArray.count, inSection: indexPath.section))
        self.delegate?.cyclePictureView(cyclePictureView: self, didSelectItemAtIndexPath: IndexPath.init(item: indexPath.item % self.imageBox!.imageArray.count, section: indexPath.section) as NSIndexPath)
    }
