    ...
    var imageSource: ImageSource = ImageSource.Local(name: ""){
        didSet {
            ...
            case let .Network(urlStr):
                let encodeString = urlStr.addingPercentEncoding(withAllowedCharacters: NSCharacterSet.urlQueryAllowed)// urlStr.stringByAddingPercentEncodingWithAllowedCharacters(NSCharacterSet.URLQueryAllowedCharacterSet())!
//                self.imageView.sd_setImageWithURL(NSURL(string: encodeString)!, placeholderImage: placeholderImage)
                self.imageView.af_setImage(withURL: URL.init(string: encodeString!)!)
            }
        }
    }
